...
NAVDEEP SINGH DHARIWAL <2018ucp1389@mnit.ac.in>
Attachments
    ...
    if(!pq.empty()){
      ...
      if(vtask.size()){
        for(int ak=0;ak < vtask.size();ak++){
          cout << "\nTask " << vtask[ak].id << " Task Executed "<< vtask[ak].e << " Period " << vtask[ak].p << " Deadline " << vtask[ak].d << endl;;
        }
      }

    for(int j=0;j<vtask.size();j++){
      if((vtask[j].d) < i){
        cout << "\nTask " << vtask[j].id << " Deadline " << vtask[j].d << " Expired" <<endl;
        return false;
      }
    }
  }
